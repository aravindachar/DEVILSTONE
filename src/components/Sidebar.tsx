    ...
    <aside className="w-full md:w-80 flex-shrink-0 flex flex-col bg-white/60 backdrop-blur-md border-r border-slate-200/80 h-full overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3">
        {sessions.map((session, idx) => {
          ...
          const colors = ['glass-card-cyan', 'glass-card-pink', 'glass-card-yellow', 'glass-card-blue'];
          const colorClass = colors[idx % colors.length];
          
          const getProgressBarColor = (percent: number) => {
            if (percent === 100) return 'bg-emerald-500';
            if (colorClass === 'glass-card-cyan') return 'bg-cyan-500';
            if (colorClass === 'glass-card-pink') return 'bg-rose-500';
            if (colorClass === 'glass-card-yellow') return 'bg-amber-500';
            return 'bg-blue-500';
          };

          return (
            <div key={session.id} className={`border rounded-2xl overflow-hidden shadow-sm transition-all duration-300 hover:shadow-md ${colorClass}`}>
              {/* Session Accordion Toggle */}
              <button
                onClick={() => toggleSession(session.id)}
                className="w-full p-4 text-left flex items-start justify-between gap-3 hover:bg-slate-900/5 transition-colors"
              >
                <div className="flex-1 min-w-0">
                  ...
                  <div className="mt-2 flex items-center gap-2">
                    <div className="flex-1 bg-slate-200/60 h-1.5 rounded-full overflow-hidden">
                      <div
                        className={`${getProgressBarColor(session.progressPercent)} h-full rounded-full transition-all duration-500`}
                        style={{ width: `${session.progressPercent}%` }}
                      />
                    </div>
                    <span className="text-[10px] font-bold text-slate-600 min-w-[24px] text-right">
